...
class CrypTensor(object, metaclass=CrypTensorMetaclass):
    ...
    def __torch_function__(self, func, types, args=(), kwargs=None):
        ...
        if func in STATIC_FUNCTION_MAPPING:
            ...
            output = args[0].matmul(args[1].t().cuda())
            if 'bias' in kwargs:
            ...
            output = args[0].softmax(kwargs['dim'])
            return output
            ...
            output2 = torch.nn.functional.one_hot(output2, 2)
            assert output1.size() == output2.size(), "input and target must have the same size"
            return output1.cross_entropy(output2, skip_forward=False)
        else:
